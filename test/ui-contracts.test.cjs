...
test('mobile header preserves the full brand and stacks actions on narrow screens', () => {
  const style = fs.readFileSync('./style.css', 'utf8');
  assert.match(style, /\.brand\{flex:0 0 auto;white-space:nowrap\}/);
  assert.match(style, /@media\(max-width:600px\)\{\.topbar\{[^}]*display:grid;[^}]*grid-template-columns:1fr;/);
});

test('mode switch dialog describes clearing the current canvas', () => {
  const html = fs.readFileSync('./index.html', 'utf8');
  const account = fs.readFileSync('./account.js', 'utf8');
  assert.match(html, /id="saveChoiceMessage"/);
